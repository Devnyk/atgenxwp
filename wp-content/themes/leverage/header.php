...
if ( function_exists( 'ACF' ) ) {
	...
	if ( get_field( 'override_general_settings' ) ) {
		$theme_mode  = get_field( 'theme_mode' );
...
<?php 
if ( $theme_mode === 'dark' ) { 
	$body_class = 'theme-mode-dark';

} else { 
	$body_class = 'theme-mode-default';
}

if ( function_exists( 'ACF' ) ) {
	if ( get_field( 'override_general_settings' ) ) {
		$body_class .= ' conditional-settings';
	}
}
?>

<body <?php body_class( $body_class ); ?> <?php if ( $body_bg_color ) { echo 'style="background-color:'.esc_attr( $body_bg_color ).'"'; } ?>>
			...
			if ( function_exists( 'ACF' ) ) :
				...
				<ul class="navbar-nav action <?php echo esc_attr( $navbar_call_to_action ); ?>">
					<li class="nav-item ml-3">
						<a href="<?php echo esc_url( $url ); ?>" <?php if ( $target == 'External Link' ) { echo esc_attr( 'target="_blank"' ); } ?> class="<?php if ( $target == 'Anchor Link' ) { echo esc_attr( 'smooth-anchor' ); } ?> btn ml-lg-auto <?php if ( $button_mode === 'outlined' ) { echo 'dark-button'; } else { echo 'primary-button'; } ?>">
							...
							if ( $button_label ) {
								echo leverage_translate( $button_label );
							} ?>
						</a>
					</li>
				</ul>
