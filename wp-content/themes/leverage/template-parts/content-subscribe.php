...
<section id="<?php echo esc_attr( $subscribe_id ); ?>" class="<?php echo esc_attr( $subscribe_class ); ?> <?php echo esc_attr( $subscribe_vs.' '.$subscribe_vsr ); ?> <?php if ( $subscribe_enable_dark_mode ) { echo esc_attr( 'odd' ); } ?> <?php if ( $subscribe_enable_separator_line ) { echo esc_attr( 'featured' ); } ?> subscription" <?php echo 'style="background-color:'.esc_attr( $subscribe_background_color ).'"'; ?>>
    <div class="container <?php the_field( 'subscribe_container', 'option' ); ?>">
		...
		$subscribe_heading_style = get_field( 'subscribe_heading_style', 'option' );
		$subscribe_description   = get_field( 'subscribe_description', 'option' );
		$subscribe_text_align    = get_field( 'subscribe_text_align', 'option' );
					
		if ( $subscribe_title || $subscribe_description ) : ?>

			<div class="row intro">
				<div class="col-12 <?php esc_attr( $subscribe_text_align ); ?> text-center">

					<?php 
					if ( $subscribe_title ) : ?>

						<h2 class="<?php if ( ! $subscribe_description ) { echo 'mb-0 '; } echo esc_attr( $subscribe_heading_style ); ?>"><?php echo leverage_translate( $subscribe_title ); ?></h2>

					<?php endif; ?>

					<?php if ( $subscribe_description ) { echo '<p>' . leverage_translate( $subscribe_description ) . '</p>'; } ?>
					...
					<div class="col-12 col-lg-2 m-lg-0 input-group align-self-center item">
						<a class="btn primary-button w-100">
							...
							if ( $subscribe_button_label ) {
								echo leverage_translate( $subscribe_button_label );

							} else { 
								echo esc_html( 'SUBSCRIBE', 'leverage' );
							} ?>
						</a>
					</div>
